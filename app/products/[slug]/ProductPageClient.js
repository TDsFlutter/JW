...
function ProductDetail({ product }) {
  ...
  const { productReviewsEnabled, discountTiers } = useSettings();
  const [selectedImage, setSelectedImage] = useState(0);

  // Plating colours come from the product's enabled colour variants. Products
  // sold in a single colour show no swatch selector (only one option), per
  // requirement. Legacy products without variants default to one plating.
  const enabledMetals = Array.isArray(product.colorVariants)
    ? product.colorVariants.filter((v) => v.enabled).map((v) => v.key)
    : [];
  const productMetals = enabledMetals.length > 0 ? enabledMetals : ["Sterling Silver"];
  const showMetalSelector = productMetals.length >= 2;
  ...
  // Discount badges are managed site-wide in the admin Settings tab. Only the
  // ones marked enabled are shown.
  const offerBadges = (Array.isArray(discountTiers) ? discountTiers : []).filter(
    (o) => o && o.enabled !== false && o.label
  );
      ...
      <div className={styles.container}>
        ...
        <div className={styles.productLayout}>
          ...
          >
            ...
            {/* Metal / Plating Selection — swatches shown only when 2+ colours */}
